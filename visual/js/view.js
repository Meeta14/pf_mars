...
var View = {
    ...
    setHillAt: function(gridX, gridY, value) {
        var node, i, hillNodes = this.hillNodes;
        if (!hillNodes) {
            hillNodes = this.hillNodes = new Array(this.numRows);
            for (i = 0; i < this.numRows; ++i) {
                hillNodes[i] = [];
            }
        }
    ...
    clearHillNodes: function() {
        var i, j, hillNodes = this.hillNodes;
        if (!hillNodes) {
            return;
        }
        for (i = 0; i < this.numRows; ++i) {
